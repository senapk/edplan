...
def colour(text: str,
  ...
  return (Color.map[color] + ("" if color2 is None else Color.map[color2]) +
          text + Color.map["reset"])
...
class Play:
  ...
  def __init__(self, game: Game, rep: Rep, jsonfile: str):
  ...
  def save_to_json(self):
    ...
    self.rep.save_to_json(self.jsonfile)
  ...
  def print_quest(self, entry, q, max_title, term_size):
    ...
    entry = colour(entry, "blue") if q.type == "main" else colour(
        entry, "magenta")
    qlink = ""
    if self.show_url:
  ...
  def print_task(self, t, max_title, index, term_size):
    ...
    if self.show_url:
      ...
      if term_size > self.term_limit:
        ...
        vlink = "\n      " + vlink
    print(
        f"  {vindex} [{vdone}] {t.title.strip().ljust(max_title + 1)}{vlink}")
  ...
  def play(self):
    while True:
      ...
      vall = colour(
          ":a", "cyan") + colour("ll") + ("[x]" if self.show_all else "[ ]")
      vurl = colour(
          ":u", "cyan") + colour("rl") + ("[x]" if self.show_url else "[ ]")
...
def create_diag(quests, output, colors=None):
  ...
  def info(q):
    color = "lime" if q.type == "main" else "pink"
  ...
  for q in quests.values():
    ...
    if len(q.requires_ptr) > 0:
      ...
      v = "Início"
      saida.append(f"{v} #cyan {token} {info(q)}")
...
def main():
  ...
  parser_init.add_argument("--url",
                           "-u",
                           type=str,
                           help="Inicia um jogo na pasta atual passando a url")
  parser_init.add_argument("--file",
                           "-f",
                           type=str,
                           help="Inicia um jogo na pasta atual passando a url")
  parser_init.add_argument(
      ...
      "-r",
      type=str,
      help="Inicia um jogo na pasta atual passado o repositório do tko")
